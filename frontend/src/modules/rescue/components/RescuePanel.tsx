...
export function RescuePanel({ sosRequests, teamStatuses, onRefresh, onActiveMissionChange }: RescuePanelProps) {
  const { user } = useAuth();
              ...
              {sos && (
                <>
                  <div className="flex items-start justify-between">
                    ...
                    <button onClick={() => panTo({ lat: sos.lastKnownLat ?? sos.lat, lng: sos.lastKnownLng ?? sos.lng }, 15)}
              ...
              {pending.map((s) => (
                <div key={s.id} className="bg-red-50 border border-red-100 rounded-2xl p-3 space-y-2">
                  <div className="flex items-start justify-between">
                    ...
                    <button onClick={() => panTo({ lat: s.lastKnownLat ?? s.lat, lng: s.lastKnownLng ?? s.lng }, 15)}
                      className="p-1.5 hover:bg-red-100 rounded-lg cursor-pointer flex-shrink-0">
