...
export function RescueTeamLayer({ teamStatuses, sosRequests, safePlaces = [], activeRescuerId }: RescueTeamLayerProps) {
  ...
  useEffect(() => {
    ...
    // Leg 1: rescuer → victim (only if rescuer has a known position)
    ...
    // Leg 2: victim → safe place shelter
  ...
  return (
    <>
      {/* Pending SOS requests — visible to all before anyone accepts */}
      {sosRequests
      ...
      {/* Assigned SOS requests — someone is on the way */}
      {sosRequests
